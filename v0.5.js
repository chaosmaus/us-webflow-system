$(document).ready(function () {
  ...
  const priceController = (availableDates, propertiesData) =>{
/*     let properties = []
    data.availability.forEach((element, index)=>{
      // id is repeated


      if(!(data.unavailableDates.includes(element.listingId))){
          properties.push({
    ...
    availableDates.forEach((element, index) =>{
      propertiesData.forEach((el, i) =>{
        if( element === el.listingId){
          if((properties[index].id  === '')) properties[index].id = element;
          properties[index].datePrice.push({
            date: el.date,
            price: el.price
          })
        }
      });
      console.log('property added: ', properties[index])
  ...
  const filterController = (data) => {
    ...
    priceController(availableDates, data.availability);
