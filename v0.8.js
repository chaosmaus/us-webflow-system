    ...
    parsedProperties.forEach((element, index) =>{
        $('.items').each((i, el)=>{
          if($(el).find('item_guesty-id').text() === element.id){
            console.log('id match', $(el).find('item_guesty-id').text())
            let totalPrice = 0;
            element.datePrice.each((intraEl, i2)=>{
              totalPrice += intraEl.price
            })
            console.log('id: ', element.id);
            console.log('total price: ',totalPrice );
            $(el).find('.apartment-pricing__price').text(totalPrice/(element.datePrice.length))
          }
